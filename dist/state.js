...
export function getDb() {
  ...
  `);
    // Migrations
    const columns = rawDb.all("PRAGMA table_info(agents)");
    ...
    db = {
        ...
        prepare: (sql) => ({
            run: (...params) => rawDb.run(sql, normalizeParams(params)),
            get: (...params) => rawDb.get(sql, normalizeParams(params)),
            all: (...params) => rawDb.all(sql, normalizeParams(params)),
        }),
    };
    return db;
}

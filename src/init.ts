...
import { AGENTD_DIR, DEFAULT_HOST, DEFAULT_PORT } from "./config.ts";
...
export function detectGitRepos(searchDirs?: string[]): string[] {
  const home = homedir();
...
export function detectGithubOwner(): string | null {
  // Try gh CLI first
  try {
...
async function checkDaemonTools(): Promise<string[] | null> {
  try {
    const res = await fetch(`${BASE_URL}/tools`);
    if (!res.ok) return null;
    const tools = (await res.json()) as { serverName: string }[];
...
async function registerAgent(name: string): Promise<{ ok: boolean; message: string }> {
...
export async function runInit(): Promise<void> {
  ...
  if (availableServers) {
    const needed = new Set(selected.flatMap((t) => {
      ...
      const parsed = parseYaml(content) as { tools?: string[] };
      return parsed.tools ?? [];
    }));

...
import { convertToolsToOllamaFormat, runOllamaLoop } from "./ollama.ts";

const MAX_RETRIES = 3;
const DEFAULT_RETRY_DELAY_MS = 60_000;
const MAX_TOKENS = 8192;

type ModelProvider = { type: "anthropic"; model: string } | { type: "ollama"; model: string };

export function parseModelProvider(model: string): ModelProvider {
  ...
  throw new Error(
    `Unknown model "${model}". Model must start with "claude-" or "ollama/" (e.g. "claude-sonnet-4-20250514", "ollama/llama3.3:70b")`,
  );
}
...
function resolveTools(agent: { tools: string[] }): RegisteredTool[] {
...
async function runAnthropicLoop(opts: {
  apiKey: string;
  agent: { model: string; prompt: string };
  resolvedTools: RegisteredTool[];
  context: string | undefined;
  runId: string;
}): Promise<{ output: string; totalToolCalls: number; totalInputTokens: number; totalOutputTokens: number }> {
...
export async function runAgent(agentName: string, runId: string, context?: string): Promise<RunResult> {
  ...
  try {
    let result: { output: string; totalToolCalls: number; totalInputTokens: number; totalOutputTokens: number };
